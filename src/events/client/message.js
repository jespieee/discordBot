module.exports = {
    ...
    async execute(msg) {
        ...
        let compliments = [
            ...
            "GIGACHAD RIGHT HERE",
            "Mmmmhm, that's right.",
            "6 foot 20 inch alpha in the house",
        ...
        let maxN = insults.length;
        let maxC = compliments.length;
        if (msg.content === key.toLowerCase()) {
            // figure out whether or not it is compliment or insult
            if (negative.includes(msg.content.toLowerCase)) {
                msg.channel.send(insults[randomIntFromInterval(0, maxN - 1)]);
            }
            else if (positive.includes(msg.content.toLowerCase)) {
                msg.channel.send(compliments[randomIntFromInterval(0, maxC - 1)]);
            }
        }
    }
}